...
function solarEvent(date: Date, lat: number, lon: number, sunAltitude: number, evening: boolean): Date | null {
  // Day of year approach via NOAA's general formula.
  const J = julianDay(date) - 2451545.0;
  const M = (357.5291 + 0.98560028 * J) % 360; // mean solar anomaly
  const C =
  ...
  const Jtransit = 2451545.0 + J + 0.0053 * Math.sin(toRad(M)) - 0.0069 * Math.sin(toRad(2 * lambda));
