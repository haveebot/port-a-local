...
function SearchContent() {
  ...
  // Get matching businesses from Fuse or all if query is empty
  ...
  // Determine available categories from current results (before category filter)
  ...
  // Reset category if it no longer has results
  useEffect(() => {
  ...
  const resultLabel =
    query.trim().length >= 2
      ? `${sorted.length} ${sorted.length === 1 ? "business" : "businesses"} match "${query}"`
      : `${sorted.length} ${sorted.length === 1 ? "business" : "businesses"} in Port Aransas`;
      ...
      <section className="pt-28 pb-14 hero-gradient relative">
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-lg text-navy-200 font-light mb-8">
            Find restaurants, bars, charters, shops and more
          </p>
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><p className="text-navy-400">Loading...</p></div>}>
      <SearchContent />
    </Suspense>
  );
}
