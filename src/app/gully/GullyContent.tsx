...
interface GullyContentProps {
  /**
   * Server-rendered trending row (or null when not enough data has
   * accumulated). Rendered inside the chip section when query is
   * empty. Injected from the parent server component so the DB read
   * happens server-side.
   */
  trendingSlot?: ReactNode;
}

export default function GullyContent({ trendingSlot }: GullyContentProps) {
      ...
      {query.trim().length === 0 && (
        <section className="bg-navy-900 pb-12 relative">
          ...
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
            ...
            {/* Trending row — server-rendered from ask_gully_log.
                Shows only when 2+ unique questions have accumulated in
                the last 24h, otherwise renders null. */}
            {trendingSlot}
